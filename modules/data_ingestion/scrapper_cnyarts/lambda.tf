...
resource "aws_lambda_function" "lambda_scrape_cnyarts" {
  filename      = "./modules/data_ingestion/scrapper_cnyarts/lambda_function.zip"
  function_name = "${var.lambda_function_name}"
  role          = aws_iam_role.role_lambda_scrape_cnyarts.arn
...
# 3. Assume policy
resource "aws_iam_role" "role_lambda_scrape_cnyarts" { # role_<lambda name>
  name = "role_lambda_scrape_cnyarts"
...
# 4. Cloud watch policy to the role
resource "aws_iam_policy" "policy_lambda_scrape_cnyarts" {
  name        = "policy_lambda_scrape_cnyarts"
  path        = "/"
  description = "IAM policy for lambda_scrape_cnyarts lambda"
...
resource "aws_iam_role_policy_attachment" "lambda_scrape_cnyarts_logs" {
  role       = aws_iam_role.role_lambda_scrape_cnyarts.name
  policy_arn = aws_iam_policy.policy_lambda_scrape_cnyarts.arn
}


# Eventbridge / Cloudwatch event
resource "aws_cloudwatch_event_rule" "scrape_cnyarts_once_a_day" {
